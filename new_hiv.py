...
class HIVsim(hpb.ParsObj):
    ...
    def init_results(self, sim):
        ...
        results['n_cin_by_age_with_hiv'] = init_res('Number CIN by age among HIV+', n_rows=na, color=stock_colors[0])
        results['n_cin_by_age_no_hiv'] = init_res('Number CIN by age among HIV-', n_rows=na, color=stock_colors[0])
        results['cins_with_hiv'] = init_res('CINs among HIV+', color=stock_colors[0])
        results['cins_no_hiv'] = init_res('CINs among HIV-', color=stock_colors[1])
    ...
    def calculate_stocks(self, people):
        ...
        # Pull out those with CIN and HIV+
        cinhivinds = hpu.true((people['hiv']) & people['cin'])
        self.results['n_cin_by_age_with_hiv'][:, idx] = \
            np.histogram(people.age[cinhivinds], bins=people.age_bin_edges, weights=people.scale[cinhivinds])[0]

        # Pull out those with CIN and HIV-
        cinnohivinds = hpu.true(~(people['hiv']) & people['cin'])
        self.results['n_cin_by_age_no_hiv'][:, idx] = \
            np.histogram(people.age[cinnohivinds], bins=people.age_bin_edges, weights=people.scale[cinnohivinds])[0]
