...
n_trials = [50, 5][debug]  # How many trials to run for calibration
n_workers = [25, 1][debug]  # How many cores to use
...
def run_calib(location=None, n_trials=None, n_workers=None,
    ...
    hiv_datafile = [f'data/zambia_hiv_incidence.csv',
                    f'data/zambia_female_hiv_mortality.csv',
                    f'data/zambia_male_hiv_mortality.csv']
    art_datafile = [f'data/zambia_art_coverage_by_age_males.csv',
                    f'data/zambia_art_coverage_by_age_females.csv']

    sim = rs.make_sim(hiv_datafile=hiv_datafile, art_datafile=art_datafile, calib=True)

    datafiles = [
        f'data/zambia_cancer_cases.csv',  # Globocan
        f'data/zambia_asr_cancer_incidence.csv',
    ]
