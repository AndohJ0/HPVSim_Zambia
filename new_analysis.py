    ...
    **Example**::
    ...
    def validate_variables(self, sim):
        ...
        for rk, rdict in self.result_args.items():
            ...
            attr = None     # To avoid unbound local error with addition of new result type 
            date_attr = None
...
class cancer_detection(Analyzer):
    ...
    def __init__(self, symp_prob=0.01, treat_prob=0.01, product=None, **kwargs):
        ...
        self.survival_data = []  # To store survival information
    ...
    def apply(self, sim):
        ...
        '''
        # Get everyone with cancer; nonzero returns row and column indices.
        cancer_genotypes, cancer_inds = sim.people.cancerous.nonzero()
    
        # Create a mapping from individual index to their cancer diagnosis date.
        # Here we assume that if an individual has multiple cancer genotypes,
        # you want the first one (or you can adjust as needed).
        cancer_date_map = {}
        # Retrieve all cancer diagnosis dates using two-dimensional indexing.
        cancer_dates = sim.people.date_cancerous[cancer_genotypes, cancer_inds]
        for genotype, person_ind, c_date in zip(cancer_genotypes, cancer_inds, cancer_dates):
            # If multiple values exist for the same person, keep the first one.
            if person_ind not in cancer_date_map:
                cancer_date_map[person_ind] = c_date

        new_detections, new_treatments = 0, 0

        if len(cancer_inds) > 0:
            detection_probs = np.full(len(cancer_inds), self.symp_prob / self.dt, dtype=hpd.default_float)
            ...
            # Collect survival data for each detected case.
            for ind in is_detected_inds:
                self.survival_data.append({
                    'age': sim.people.age[ind],
                    'date_cancer': cancer_date_map.get(ind, None),  # Retrieve the correct date
                    'date_detected': sim.t,
                    'date_death': sim.people.date_dead_cancer[ind],
                    'has_hiv': sim.people.hiv[ind],
                    'treated': ind in treat_inds  # Indicate if the individual was treated
                })
    ...
    def finalize(self, sim):
        '''
        Finalize the analyzer and convert survival data to a DataFrame
        '''
        import pandas as pd
        self.survival_df = pd.DataFrame(self.survival_data)  # Convert to DataFrame
        return
